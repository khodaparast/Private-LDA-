...
public class Party1 implements MessageListener {
    ...
    public void start() throws IOException {
        ...
        try {
            ...
            Destination reaDestination = (Destination) initContext.lookup(PARTY_1_INPUT_Q);
            Destination senDestination = (Destination) initContext.lookup(PARTY_1_OUTPUT_Q);
            initContext.close();
...
//--------------------------------------------------------------------------------------------------------------------------------------
            ...
            TrainCommon trainCommon = new TrainCommon(FILE_PATH_PARTY_1);
            ...
            message.setStringProperty("sender", "p1.1");
            ...
            message.setStringProperty("sender", "p1.2");
            message.setStringProperty("type", "mean");
            sender.send(message);
...
//            covariance No
            ...
            message.setStringProperty("sender", "p1.5");
            message.setStringProperty("type", "covariance");
            message.setObject(en_cov_No_int_local);
            sender.send(message);

            message.setStringProperty("sender", "p1.6");
            message.setStringProperty("type", "covariance");
            message.setObject(en_cov_No_deci_local);
            sender.send(message);

            System.out.println("state[4] >>>>>>>>> 0 ");
            while (hashMessage.get("co.6") == null || hashMessage.get("co.7") == null) {// wait here until receive decrypted  covariance of class No
                Thread.sleep(100);
            }
