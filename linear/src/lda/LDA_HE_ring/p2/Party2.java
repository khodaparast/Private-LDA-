...
//--------------------------------------------------------------------------------------------------------------------------------------
            ...
            while (hashMessage.get("p1.1") == null) {//wait here until receive encrypted value of P1  "p1.1"
                Thread.sleep(50);
            }
            ...
            while (hashMessage.get("co.2") == null) {//wait until coordinator sends total mean of class Yes
                Thread.sleep(50);
            }
            ArrayList<BigInteger> totalAvgClass_Y = (ArrayList<BigInteger>) hashMessage.get("co.2");
            System.out.println("state[3] >>>>>>>>> 0");
            while (hashMessage.get("p1.2") == null) { // wait until party 1 sends encrypted of class No "p1.2"
                Thread.sleep(100);
            }
            ...
            while (hashMessage.get("co.3") == null) {// wait here until receive decrypted of class No
                Thread.sleep(100);
            }
...
//--------------------------------------------------------------------------------------------------------------------------------------
            ...
            System.out.println("state[4] >>>>>>>>> 0");
            while (hashMessage.get("p1.3") == null) { // wait until party 1 sends encrypted of class yInt
                Thread.sleep(50);
            }
            ArrayList<ArrayList<BigInteger>> en_cov_Yes_int_P1= (ArrayList<ArrayList<BigInteger>>) hashMessage.get("p1.3");
            ArrayList<ArrayList<BigInteger>> en_cov_Yes_int_P12=new ArrayList<>();
            for (int i = 0; i < en_cov_Yes_int_P1.size(); i++) {
                en_cov_Yes_int_P12.add(getSumOfTwoEncryptedList(en_cov_Yes_int_P1.get(i), en_cov_Yes_int_local.get(i)));
            }
            message.setStringProperty("sender", "p2.3");
            message.setStringProperty("type", "covariance");
            message.setObject(en_cov_Yes_int_P12);
            sender.send(message);

            System.out.println("state[5] >>>>>>>>> 0");
            while (hashMessage.get("p1.4") == null) { // wait until party 1 sends encrypted of class yDec
                Thread.sleep(50);
            }
            ArrayList<ArrayList<BigInteger>> en_cov_Yes_deci_P1= (ArrayList<ArrayList<BigInteger>>) hashMessage.get("p1.4");
            ArrayList<ArrayList<BigInteger>> en_cov_Yes_deci_P12=new ArrayList<>();
            for (int i = 0; i < en_cov_Yes_deci_P1.size(); i++) {
                en_cov_Yes_deci_P12.add(getSumOfTwoEncryptedList(en_cov_Yes_deci_P1.get(i), en_cov_Yes_deci_local.get(i)));
            }

            message.setStringProperty("sender", "p2.4");
            message.setStringProperty("type", "covariance");
            message.setObject(en_cov_Yes_deci_P12);
            sender.send(message);
...
//--------------------------------------------------------------------------------------------------------------------------------------
            ...
            ArrayList<ArrayList<BigInteger>> en_cov_No_deci_P1= (ArrayList<ArrayList<BigInteger>>) hashMessage.get("p1.6");
            ArrayList<ArrayList<BigInteger>> en_cov_No_deci_P12=new ArrayList<>();
            for (int i = 0; i < en_cov_No_deci_P1.size(); i++) {
                en_cov_No_deci_P12.add(getSumOfTwoEncryptedList(en_cov_No_deci_P1.get(i), en_cov_No_deci_local.get(i)));
            }

            message.setStringProperty("sender", "p2.6");
            message.setStringProperty("type", "covariance");
            message.setObject(en_cov_No_deci_P12);
            sender.send(message);
